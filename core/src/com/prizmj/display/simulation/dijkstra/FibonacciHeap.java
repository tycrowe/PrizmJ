...
public final class FibonacciHeap {
     ...
     */
    public Entry enqueue(Vertex value, Float priority) {
        checkPriority(priority);

        /* Create the entry object, which is a circularly-linked list of length
         * one.
         */
        value.setWeight(priority);
    ...
    public Entry dequeueMin() {
        ...
        for (Entry curr : toVisit) {
             ...
             * the root-level one.
             */
            if (curr.getValue().getWeight() <= mMin.getValue().getWeight()) mMin = curr;
        }
        return minElem;
    }
    ...
    public void decreaseKey(Entry entry, Float newPriority) {
        ...
        if (newPriority > entry.getValue().getWeight())
    ...
    private void decreaseKeyUnchecked(Entry entry, Float priority) {
        ...
        entry.getValue().setWeight(priority);
         ...
         * from its parent.
         */
        if (entry.mParent != null && entry.getValue().getWeight() <= entry.mParent.getValue().getWeight())
            cutNode(entry);
         ...
         */
        if (entry.getValue().getWeight() <= mMin.getValue().getWeight())
            mMin = entry;
    }
